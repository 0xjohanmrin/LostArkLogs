...
export function getPieceStatValue(
  ...
  const x = slot.replace(/\d+/, "") as string;
  const piece = x as keyof typeof QualityEffect;
  const rarityAsStr = ItemRarity[rarity];
  const eff = QualityEffect[piece];
  console.log(rarityAsStr);
  const effect = QualityEffect[piece][2];
  if (!effect) {
